...
from genweb6.organs.firma_documental.views.general import TMP_FOLDER
from genweb6.organs.firma_documental.views.general import generate_pdf_with_timeout
...
class SignActa(BrowserView, FirmesMixin):
    ...
    def generateActaPDF(self):
        if generate_pdf_with_timeout(self, self.context.absolute_url() + '/printActa', TMP_FOLDER + self.context.id + '.pdf'):
    ...
    def generateDocumentPDF(self, document, filename, visibility='public'):
        ...
        if generate_pdf_with_timeout(self, document.absolute_url() + '/printDocument?visibility=' + visibility, TMP_FOLDER + _filename):
    ...
    def __call__(self):
        ...
        try:
            ...
            if not actaPDF:
                self.context.plone_utils.addPortalMessage(
                    _(u'No s\'ha pogut generar el PDF de l\'acta'), 'error')
                logger.error('No s\'ha pogut generar el PDF de l\'acta')
                self.context.reindexObject()
                transaction.commit()
                return "Error"
...
class CancelSignatura(BrowserView, FirmesMixin):
    """
